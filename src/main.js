...
const TEACHER_PROBE_BANK_URL = new URL("../data/teacher-probe-bank.json", import.meta.url);
...
async function init() {
  try {
    const probeBankResponse = await fetch(TEACHER_PROBE_BANK_URL);
    if (!probeBankResponse.ok) {
      throw new Error(`Could not load teacher probe bank (${probeBankResponse.status})`);
    }
    state.teacherProbeBank = await probeBankResponse.json();
    validateTeacherProbeBankShape(state.teacherProbeBank);
...
function validateTeacherProbeBankShape(bank) {
  if (!bank || typeof bank !== "object" || !Array.isArray(bank.generic) || typeof bank.phases !== "object") {
    throw new Error("Teacher probe bank shape is invalid.");
  }
}
                    ...
                    <label class="checkbox-row section-card">
                      ...
                      <span class="section-card-body">
                        <span class="section-card-top">
                          ...
                          <span class="section-card-title">${escapeHtml(sectionLabel(section))}</span>
                        </span>
                      </span>
                    </label>
                  `)
...
function getTeacherProbePrompts(section, summary) {
  ...
  const generic = Array.isArray(state.teacherProbeBank?.generic) ? state.teacherProbeBank.generic : [];
  const prompts = state.teacherProbeBank?.phases?.[phaseKey]?.[section?.section_id] || generic;
